...
fn parse(p: *struct parser, t: enum token) -> void:
    if p.token == t:
        bump(p)
        return
    var msg: [64]char
    var x = token2string(t)
    var y = token2string(p.token)
    sprintf(msg, "expected %s, got %s", x, y)
    print_parse_error(p, msg)
    exit(1)

fn parse_type(p: *struct parser) -> *struct type:
    return NULL

fn parse_function_type(p: *struct parser) -> struct function_type:
    var f = (struct function_type){}
    parse(p, token_lparen)

    parse(p, token_rparen)
    parse(p, token_arrow)
    f.return_type = parse_type(p)
    return f

fn parse_function(p: *struct parser) -> void:
    parse(p, token_fn)
    var name = p.string
    parse(p, token_identifier)
    var type = parse_function_type(p)
    return
